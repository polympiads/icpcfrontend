...
import { type PDFPrintService, PDFPrintServiceFactory } from "./lib";
...
export class PDFSlickPrintService {
    ...
    #beforePrint() {
        ...
        if (!this.slick.viewer.pageViewsReady) {
            this.slick.l10n.get("printing_not_ready", null, "Printing is not ready").then((msg: any) => {
        ...
        window.addEventListener(
            ...
            (event) => {
                ...
                if (
                    ...
                    !event.altKey &&
                    (!event.shiftKey || 'chrome' in window || 'opera' in window)
                ) {
                    this.print();
