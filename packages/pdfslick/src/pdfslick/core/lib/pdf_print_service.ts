...
function getXfaHtmlForPrinting(
  ...
  const xfaHtml = pdfDocument.allXfaHtml as { children: any[] } | null;
  if (!xfaHtml) return;
...
function renderPage(
  ...
  printAnnotationStoragePromise: Promise<
    PDFDocumentProxy["annotationStorage"] | void
  >,
) {
...
class PDFPrintService {
  ...
  _printAnnotationStoragePromise: Promise<
    PDFDocumentProxy["annotationStorage"] | void
  >;
  currentPage: number;
  scratchCanvas: HTMLCanvasElement;
  pageStyleSheet: HTMLStyleElement | null = null;
  ...
  renderPages(
    renderProgress: (status: { index: number; total: number }) => void,
  ) {
