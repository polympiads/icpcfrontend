...
export class PDFSlick {
  ...
  constructor({
    ...
    const viewerOptions: PDFViewerOptions = {
      ...
      findController,
      renderingQueue:
        renderingQueue as unknown as PDFViewerOptions["renderingQueue"],
    ...
    if (thumbs) {
      this.thumbnailViewer = new PDFThumbnailViewer({
        ...
        renderingQueue,
        maxCanvasPixels:
          this.useOnlyCssZoom === true ? 0 : this.maxCanvasPixels,
        maxCanvasDim: this.maxCanvasDim,
  ...
  async loadDocument(
    ...
    options?: {
      filename?: string;
      onProgress?: (_: OnProgressParameters) => void;
    },
  ) {
  ...
  async #parsePageSize(
    ...
    const [{ width, height }, unit, name, orientation] = await Promise.all([
      ...
      this.l10n.get(
        ...
        null,
        (isPortrait ? "portrait" : "landscape").toLowerCase(),
      ),
    ]);
  ...
  async download() {
    ...
    try {
      ...
      await this.downloadManager?.downloadData(
        blob,
        filename,
        "application/pdf",
      );
  ...
  #initInternalEventListeners() {
    ...
    this.eventBus.on(
      "switchspreadmode",
      this.#onSwitchSpreadMode.bind(this),
      opts,
    );
    this.eventBus.on(
      "switchscrollmode",
      this.#onSwitchScrollMode.bind(this),
      opts,
    );
  }
