...
import type { L10n as IL10n, PDFViewerOptions } from "pdfjs-dist/types/web/pdf_viewer";
import type {
    TEventBusEvent,
    PDFSlickInputArgs,
    PDFSlickState,
...
export class PDFSlick {
    ...
    #thumbsContainer: HTMLElement | undefined;
    ...
    async #parsePageSize(
        ...
        const [{ width, height }, unit, name, orientation] = await Promise.all([
            ...
            this.l10n.get(
                `document_properties_page_size_unit_${_isNonMetricLocale ? "inches" : "millimeters"}`, null, _isNonMetricLocale ? "inches" : "millimeters"
            ),
            rawName &&
            this.l10n.get(
                `document_properties_page_size_name_${rawName.toLowerCase()}`, null, rawName
            ),
            this.l10n.get(
                `document_properties_page_size_orientation_${isPortrait ? "portrait" : "landscape"}`, null, (isPortrait ? "portrait" : "landscape").toLowerCase()
            ),
        ]);
    ...
    async download() {
        ...
        try {
            ...
            const blob = new Uint8Array( await new Blob([data], { type: "application/pdf" }).arrayBuffer() );;
    ...
    #onPageRendered({ pageNumber }: TEventBusEvent) {
