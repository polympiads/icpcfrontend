...
export class PDFSlickPrintDialog implements IPDFSlickPrintDialog {
    ...
    if (!dialog) {
      ...
      dialog = h(
        "dialog",
        {
          ...
          class: "pdfSlick-dialog",
          style: "min-width: 200px;",
        },
        [
          h("div", { class: "row" }, [
            h("span", { "data-l10n-id": "pdfjs-print-progress-message" }, [
          ...
          h("div", { class: "buttonRow" }, [
            h(
              "button",
              { id: "printCancel", class: "dialogButton", type: "button" },
              [
                h(
                  "span",
  ...
  async open(options?: DialogOpenProps) {
    const btnClose =
      this.dialog.querySelector<HTMLButtonElement>("#printCancel");
  ...
  progress({ index, total }: { index: number; total: number }) {
    ...
    const progressBar =
      this.dialog.querySelector<HTMLProgressElement>("progress")!;
    const progressPerc =
      this.dialog.querySelector<HTMLElement>(".relative-progress")!;
