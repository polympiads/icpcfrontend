...
export function parseContest(contest: ContestJson) {
  const result: Contest = {
    ...
    countdown_pause_time: contest.countdown_pause_time
      ? parseReltime(contest.countdown_pause_time)
      : undefined,
    duration: parseReltime(contest.duration),

    scoreboard_freeze_time: contest.scoreboard_freeze_time
      ? parseTime(contest.scoreboard_freeze_time)
      : undefined,
    scoreboard_thaw_time: contest.scoreboard_thaw_time
      ? parseTime(contest.scoreboard_thaw_time)
      : undefined,
...
export function contestEquals(
  j1: Contest | undefined,
  j2: Contest | undefined,
) {
  if (j1 === undefined && j2 === undefined) return true;
  if (j1 === undefined || j2 === undefined) return false;

  return (
    j1.countdown_pause_time == j2.countdown_pause_time &&
...
export function contestStateEquals(
  j1: ContestState | undefined,
  j2: ContestState | undefined,
) {
  if (j1 === undefined && j2 === undefined) return true;
  if (j1 === undefined || j2 === undefined) return false;

  return (
    j1.end_of_updates == j2.end_of_updates &&
    j1.ended == j2.ended &&
