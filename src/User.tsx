...
export function UserLoginWidget() {
  ...
  const textInputValidationState = () =>
    loginFailedReason() !== undefined ? "invalid" : "valid";
  ...
  function onLogin() {
    getLoginTask().spawnTask(
      login(usernameInput.textContent, passwordInput.textContent),
    );
  }
      ...
      <Popover.Portal>
        <Popover.Content class="w-60 bg-white rounded-md border border-black/10 shadow-md outline-0 px-3 flex flex-col items-center pb-2">
          ...
          <Show when={userInfo() !== undefined}>
            ...
            <Button
              class="bg-red-500 flex flex-row items-center p-2 px-6 rounded-md cursor-pointer ui-disabled:bg-red-500 ui-disabled:text-white/60"
              onClick={onLogout}
            >
              <LogOut class="mr-2" />
          ...
          <Show when={userInfo() === undefined}>
            ...
            <TextField
              class="flex flex-row w-full p-0.5 overflow-hidden text-xl border border-gray-500 focus-within:border-black rounded-md mb-2 cursor-text ui-disabled:border-gray-400 ui-disabled:text-gray-600 ui-invalid:border-red-500"
              disabled={isLogging()}
              validationState={textInputValidationState()}
            >
              <TextField.Label class="mr-2 cursor-text">
                <FaSolidUserAlt class="h-6 w-6 object-cover m-2" />
              </TextField.Label>
              <TextField.Input
                ref={usernameInput}
            ...
            <TextField
              class="relative flex flex-row w-full p-0.5  text-xl border border-gray-500 focus-within:border-black rounded-md mb-4 cursor-text ui-disabled:border-gray-400 ui-disabled:text-gray-600 ui-invalid:border-red-500"
              disabled={isLogging()}
              validationState={textInputValidationState()}
            >
              <TextField.Label class="mr-2 cursor-text">
                <CgPassword class="h-6 w-6 object-cover m-2" />
              </TextField.Label>
              <TextField.Input
                ref={passwordInput}
            ...
            <Button
              class="bg-sky-300 flex flex-row items-center p-2 px-6 rounded-md cursor-pointer ui-disabled:bg-sky-200 ui-disabled:text-gray-600"
              onClick={onLogin}
              disabled={isLogging()}
            >
              <LogIn class="mr-2" />
